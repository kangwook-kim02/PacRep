...
class TextDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, filepath, chunk_size, max_len_text, config_label):
        ...
        self.chunk_size = chunk_size # 한 번에 읽을 파일 크기
...
class IterTextDataset:
    def __init__(self, filepath, chunk_size, config, config_label, tackle_data=None, num_workers=4, shuffle=True,
        ...
        self.dataset = TextDataset(filepath, chunk_size, config.max_length_sen, config_label) # TextDataset 객체 생성
        self.batch_size = config.batch_size
        self.tackle_data = tackle_data # text_collate_fn
    ...
    def __reset__(self):
        ...
        '''
        # TextDataset이 가지고 있는 청크 바탕으로 DataLoader 객체 생성
        ...
        self.data_loader_iter = self.data_loader.__iter__() # DataLoader iter

    def get_data(self):
        try:
            # 학습용 배치를 하나 꺼냄
            data = next(self.data_loader_iter) # hjroh: from pytorch 1.13 https://stackoverflow.com/questions/74289077/attributeerror-multiprocessingdataloaderiter-object-has-no-attribute-next
