...
class TrainModel(object):
    # config: 학습에 필요한 모든 설정(경로, 파라미터)
    # device: CPU, GPU 결정
    def __init__(self, config, device=torch.device('cpu')):
        self.dataset_name_eva = ('train', 'valid', 'test') if config.has_valid else ('train', 'test') # 데이터셋 종류 정함 valid 있으면 전, 없으면 후
        self.config = config
        self.num_loss = config.n_loss # 모델이 계산할 loss 종류 개수 저장 (분류 손실, 대조 손실 등)
        logger.info('Model parameters: %s' % str(config)) # 설정된 파라미터 로그로 남김
        data_loader_num_worker = 4 if config.gpu_mode > 0 else 0 # 데이터 불러올 때 사용할 코어 수 정함
        logger.info('Num of worker in data loader is: %s' % data_loader_num_worker) 
        config_label = dict() # task별 레이블 정보 담을 딕셔너리
        config.tasks_label = json.load(open(config.label_path, 'r')) # task 별 label 정보 불러옴
        logger.info('task detail: %s' % config.tasks_label)
        config_label['detail'] = config.tasks_label # 읽은 label 정보를 딕셔너리에 저장
        config_label['n_task'] = len(config_label['detail'])
        self.config_label = config_label
        self.model = RecognitionModel(config, device=device) # config에 지정된 모델 객체 생성
        self.data = {}
        self.data_train = IterTextDataset( # IterTextDataset: 데이터 불러오는 객체
            os.path.join(config.data_dir, 'train.txt'),
            chunk_size=1024 * 1024 * 256,
            config=config,
            config_label=config_label,
            tackle_data=text_collate_fn, # text_collate_fn..
        ...
        for tmp in self.dataset_name_eva:
            with open(os.path.join(config.data_dir, '%s.txt' % tmp), 'r') as f:
                self.data[tmp] = build_data_for_eva(
                    # f.readlines(1024 * 1024 * 64), # 64MB까지만 읽음 수정 20250731 kangwook kim
                    f.readlines(),
                    max_length_text=config.max_length_sen,
                    config_label=config_label,
                    sampled_num=config.sampled_num, # 모든 test를 평가하기 위해서
                    # sampled_num = -1, # 학습 데이터 양 정하고 싶으면 이거 주석 처리 하고 위에 있는 코드 써야함!!
                )
    ...
    def run(self, config):
        ...
        if config.breakpoint > 0: # breakpoint 설정하여 학습 진행 시 저장된 모델 불러옴
        ...
        total_step = config.max_checkpoint * config.per_checkpoint
        for step in tqdm(range(start_iter, total_step), initial=start_iter, total=total_step, desc="Training"):
            if step % config.per_checkpoint == 0: # step이 256번마다 -- per_checkpoint = 256
                if self.config.gpu_mode == 3:
                    if torch.distributed.get_rank() > 0:
                        continue
                show = lambda a: '[%s]' % (' '.join(['%.4f' % x for x in a]))
                n_iter = int(step / config.per_checkpoint) # n_iter는 평가 횟수
                ...
                # if config.breakpoint < 0: # 주석처리 진행
                #     exit()
            loss_step = loss_step + TrainModel.train(self.model, self.data_train) / config.per_checkpoint # 모델 학습 (배치사이즈만큼)
    ...
    def evaluate(model, data, batch_size, num_loss, config_label):
        ...
        dict_eva = dict()
        for task in dict_all_label.keys(): # 6가지 태스크에 대해 설정되어 있음(label.config) 
