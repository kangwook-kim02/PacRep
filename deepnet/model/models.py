...
class BertConCap(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.add_module('bert', WordBertNet(
            bert_config_path=os.path.join(CONST.APP_ROOT_PATH, config.bert_config_path), #/PacRep/config/uncased_L-12_H-768_A-12/bert_config.json
            pretrained_model_path=os.path.join(CONST.APP_ROOT_PATH, config.bert_pretrained_path), #/PacRep/config/uncased_L-12_H-768_A-12/vocab.txt
        ...
        for task, label_item in config.tasks_label.items():
            if not label_item['is_valid']:
                continue
            n_label = label_item['value'] # label.conf 파일에서 value는 label의 수를 의미함
    ...
    def cal_hinge_loss(dict_pred, ground_truth, mask, tasks_detail):
        ...
        for task, prob in dict_pred.items():
            ...
            loss_total_task = loss_total_task + contribution * batch_loss # label.conf 파일에서 contribution은 가중치
