...
parser.add_argument('--learning_rate', type=float, default=1e-3) # 학습률
parser.add_argument('--lr_word_vector', type=float, default=1e-4) 
parser.add_argument('--lr_bert', type=float, default=1e-5) # bert 모델 학습률
parser.add_argument('--weight_decay', type=float, default=0)
parser.add_argument('--max_length_sen', type=int, default=256) #text의 최대 길이 PacRep 논문 마지막 내용 참고
...
parser.add_argument('--optim_type', type=str, default="Adam", choices=["Adam", "Adadelta", "Adagrad"]) # 경사하강법과 같이 가중치 값을 바꾸는 최적화 알고리즘
...
if __name__ == "__main__":
    ...
    # 동일한 결과를 얻기 위해 seed값을 설정 *위에 코드에서 파이썬을 실행할 때 seed값을 설정할 수 있음
    np.random.seed(config.seed)
    random.seed(config.seed)
    torch.manual_seed(config.seed)
    log_file_path = os.path.join(os.path.dirname(__file__), 'log') #./log/ 경로
    config.data_dir = os.path.join(os.path.dirname(__file__), config.data_dir) #상대 경로를 절대 경로로 변경
    logging.basicConfig(
